...
/**
 * Shared deploy implementation (service + sync helpers). Does not add `--sync-config-file-path`;
 * extend {@link DeployCommandBaseWithSyncPath} for commands that deploy or validate local sync YAML.
 */
export default class DeployCommandBase extends CloudInstanceCommand {
  static flags = {
